...
async function deleteRecipeById(collectionName, documentObjectId) {
    ...
    try {
        ...
        if (deleteResult.deletedCount === 1) {
            return { message: `Recipe with ObjectId ${documentId} deleted successfully.` };
        } else {
            throw new Error(`Recipe with ObjectId ${documentId} not found.`);
        }
    } catch (error) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    //Check if the request contains an ID for the recipe to be deleted
    if (!requestData) {
        throw new Error('Recipe ID is required for deletion.');
    }

    // Call the deleteDocumentById function to delete the user
    const result = await deleteRecipeById("Resepti", requestData);
